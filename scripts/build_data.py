...
def build_barcode_to_pcode_lookup(master_path):
    ...
    for r in range(2, ws.max_row + 1):
        parent_sku = ws.cell(row=r, column=8).value       # kolom H: Parent SKU
        parent_barcode = ws.cell(row=r, column=10).value  # kolom J: Parent Product Barcode
        if parent_barcode is not None and parent_sku is not None:
            lookup[str(parent_barcode).strip()] = str(parent_sku).strip()
    return lookup
...
def build_sku_lists(group_path, barcode_to_pcode):
    wb = openpyxl.load_workbook(group_path, data_only=True)
    results = {}
    for sheet_name, out_name in SHEET_TO_OUTPUT.items():
        ...
        for r in range(2, ws.max_row + 1):
            ...
            items.append({
                'barcode': bstr,
                'pcode': barcode_to_pcode.get(bstr),  # None kalau tidak ketemu di master data
...
def main():
    ...
    barcode_to_pcode = build_barcode_to_pcode_lookup(SOURCE_DIR / 'PH_SKU_AUDIT.xlsx')
    ...
    sku_lists = build_sku_lists(SOURCE_DIR / 'COTC_BY_GROUP.xlsx', barcode_to_pcode)
    for out_name, items in sku_lists.items():
        ...
        print(f"{out_name}.json: {len(items)} SKU ({no_pcode} tanpa PC Code hasil matching)")
